package com.trio.stride.ui.theme

import androidx.compose.ui.graphics.Color

object StrideColor {
    val transparent: Color = Color(0x00FFFFFF)
    val primary600: Color = Color(0xFF006699)
    val secondary: Color = Color(0xFFD10A46)
    val secondary400: Color = Color(0xFFE90C56)
    val gray: Color = Color(0xFF666666)
    val gray500: Color = Color(0xFF878787)
    val gray600: Color = Color(0xFF555555)
    val gray300: Color = Color(0xFFDDDDDD)
    val gray200: Color = Color(0xFFE8E8E8)
    val green600: Color = Color(0xFF068426)
    val background: Color = Color(0xFFF0F0F0)
    val white: Color = Color(0xFFFFFFFF)
    val red500: Color = Color(0xFFEA4843)
    val red400: Color = Color(0xFFEF7476)
    val red600: Color = Color(0xFFDF2824)
    val red700: Color = Color(0xFFB81506)
    val red800: Color = Color(0xFF890E11)

}